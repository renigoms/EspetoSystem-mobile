...
class _ClientPaymentsPageState extends State<ClientPaymentsPage> {
  ...
  void initState() {
    super.initState();
    _showcaseView = ShowcaseView.register(scope: 'client_payments');
    Future.microtask(() {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!viewModel.isLoading && !viewModel.hasSeenPaymentsOnboarding && !_tutorialStarted) {
        ...
        ShowcaseView.getNamed('client_payments').startShowCase(keys);
      ...
      children: [
        ...
        if (payments.isNotEmpty)
          CustomShowcase(
            ...
            scope: 'client_payments',
            title: isPaid ? 'Conta Paga' : 'Status da Conta',
            description: isPaid
                ? 'A conta deste cliente está totalmente quitada! O card verde indica que não há pendências.'
                : 'Veja se a conta deste cliente está PAGA ou se ainda há saldo pendente.',
